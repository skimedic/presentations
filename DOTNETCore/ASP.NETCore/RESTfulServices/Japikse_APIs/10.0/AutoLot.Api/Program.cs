...
builder.Services.AddOpenApi("v2-Beta", options =>
{
    options.AddDocumentTransformer((
        document,
        context,
        cancellationToken) =>
    {
        document.Info = BuildDocumentInfo("v2-Beta", false);
        return Task.CompletedTask;
    });
    options.OpenApiVersion = OpenApiSpecVersion.OpenApi3_1;
    options.ShouldInclude = (
...
builder.Services.AddOpenApi("v2.0-Beta", options =>
{
    options.AddDocumentTransformer((
        document,
        context,
        cancellationToken) =>
    {
        document.Info = BuildDocumentInfo("v2.0-Beta", false);
        return Task.CompletedTask;
    });
    options.OpenApiVersion = OpenApiSpecVersion.OpenApi3_1;
    options.ShouldInclude = (
